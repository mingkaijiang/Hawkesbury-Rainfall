...
R05PS_pred<-function(Datfile, sourceDir = DAILY.DATA.DIRECTORY, destDir = DAILY.OUTPUT.DIRECTORY)
{
    # prepare file list
    ...
    v.range <- c(X$r05p_spr, X$r05p_sum, 
                 X$r05p_aut, X$r05p_win)
    ...
    if(diff > 0) {
        ...
        spr_cut = cut(X[, "r05p_spr"], breaks, include.lowest=TRUE,right=TRUE)
        sum_cut = cut(X[, "r05p_sum"], breaks, include.lowest=TRUE,right=TRUE)
        aut_cut = cut(X[, "r05p_aut"], breaks, include.lowest=TRUE,right=TRUE)
        win_cut = cut(X[, "r05p_win"], breaks, include.lowest=TRUE,right=TRUE)
